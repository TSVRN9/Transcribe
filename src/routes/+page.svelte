...
<main class="mx-auto flex max-w-xl flex-col gap-4 px-4 pb-12">
	<div class="flex gap-2 rounded-xl bg-slate-800/50 p-4">
		...
		>
		<button
			class="{modeBtn} {mode === 'youtube' ? 'bg-blue-600' : 'bg-slate-700 hover:bg-slate-600'}"
	...
	{#if isReady}
		<div class="flex flex-col gap-4 rounded-xl bg-slate-800/50 p-4">
			...
			<div class="grid grid-cols-3 gap-2">
				...
				<button class={gridBtn} on:click={togglePlayback} title={tooltip('togglePlayback')}
					>{paused ? '▶️' : '⏸'}</button
				>
			...
			<div class="grid grid-cols-3 gap-2">
				...
				<button class="{gridBtn} font-mono text-sm" disabled
					>{Math.round(playbackRate * 100)}%</button
				>
